...
1. The member presses **Forgot your password** on the sign-in page. A dialog tells them to message
   the organisers, with a one-tap WhatsApp link and the first message already written for them.
...
**Admin → Settings → Password reset contact.** Put in the WhatsApp number of whoever is covering
resets and, optionally, their name — the button then reads *"Message Amana on WhatsApp"*. Include
the country code (`+971 …`); a number starting `0` is refused rather than guessed at, because
`050 123 4567` is a valid number in a dozen countries and prepending the wrong one sends members
to a stranger. Press **Test this link** to open the chat yourself before trusting it.

Leave the number blank and the button falls back to Instagram — which is also what happens if the
database is unreachable, so a member who cannot sign in always sees a way to ask for help.

Which admin last changed it is recorded in `app_settings.updated_by`.

Run `drizzle/0001_password_reset.sql` and `drizzle/0002_app_settings.sql` once before deploying
this. Both are idempotent.
...
│   ├── settings.ts                 Admin-editable settings (reset contact)
...
- `app_settings` holds only public-facing contact details, never secrets — API keys and connection
  strings stay in environment variables, out of reach of an admin page and out of database backups.
